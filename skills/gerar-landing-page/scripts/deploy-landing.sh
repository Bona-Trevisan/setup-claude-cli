...
#
# REQUISITOS:
#  - .env carregado (em /opt/steve-agent/.env OU ./.env)
...
# ---------------------------------------------------------------
ENV_LOADED=0
for ENV_FILE in "/opt/steve-agent/.env" "./.env" "$(dirname "$0")/../../.env"; do
  if [[ -f "$ENV_FILE" ]]; then
    set -a
...
1. Edite /opt/steve-agent/.env (ou o .env do seu projeto) e adicione:
